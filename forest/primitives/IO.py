...
class CsvReadPrim(Primitive):
    ...
    def __call__(self, filename = None):
        ...
        with open(self.filename) as csvfile:
            ...
            data=[]
            #boolean to check whether the data is spatio temporal
            isspatiotemporal=False
            i=0
            for row in reader:
                #this mapping should come from global parameters, we are assuming lat,lon to be y,x
                lat,lon=float(row['y']),float(row['x'])
                lon_t,lat_t=transform(source,target,lon,lat)
                #print (str(lon)+'  '+str(lat))
                timeval=None
                #this mapping should come from global parameters we are assuming time to be  t
                if 't' in row:
                    timeval=row['t']
                #print (str(point.GetX())+'  '+str(point.GetY()))
                #for calculating the x,y bounds
                if lon_t<minx:
                    minx=lon_t
                if lat_t<miny:
                    miny=lat_t
                ...
                if lat_t>maxy:
                    maxy=lat_t
                #point data as a dictionary object
                pointdat={}
                pointdat['id']=i
                pointdat['x']=lon_t
                pointdat['y']=lat_t
        ...
        print str(y)+","+str(x)+","+str(h)+","+str(w)+","+str(s)+","+str(d)+","
        layer=None
        if isspatiotemporal:
