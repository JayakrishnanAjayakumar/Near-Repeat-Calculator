...
class TileEngine(Engine):
    ...
    def split(self, bobs):
        print 'tile split '+str(self.is_split)
        ...
        for bob in Config.inputs:
            ...
            if not isinstance(bob,Raster): # Check if not a raster
                ...
                if isinstance(bob,STPoint):
                    ...
                    distancebufferinmeters,timebufferinmilliseconds=2000,183*24*60*60*1000
                    ...
                    for d in bob.data:
                        for box in tiles:
                            ...
                            if d['x']>=np.maximum((box.x-distancebufferinmeters),bob.x) and d['x']<=np.minimum((box.x-distancebufferinmeters+box.w+(2*distancebufferinmeters)),(bob.x+bob.w)) and d['y']>=np.maximum((box.y-distancebufferinmeters),bob.y) and d['y']<=np.minimum((box.y-distancebufferinmeters+box.h+(2*distancebufferinmeters)),(bob.y+bob.h)) and d['t']>=np.maximum((box.s-timebufferinmilliseconds),bob.s) and d['t']<=np.minimum((box.s-timebufferinmilliseconds+box.d+(2*timebufferinmilliseconds)),(bob.s+bob.d)):
